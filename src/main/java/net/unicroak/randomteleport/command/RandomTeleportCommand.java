...
public final class RandomTeleportCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (!(sender instanceof Player)) {
            sender.sendMessage(ChatColor.RED + "このコマンドはゲーム内でのみ実行できます");
        }

        RandomTeleporter teleporter = new RandomTeleporter(
                (Player) sender,
                randomTeleportConfig.getRadius(),
                randomTeleportConfig.getDestinationWorldList()
        );
        ...
        if (teleporter.execute()) {
            ...
            // failure
            sender.sendMessage(ChatColor.RED + "テレポートに失敗しました. 再度実行してください");
        }
